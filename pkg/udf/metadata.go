...
func allFunctionMetadata() []FunctionMetadata {
	return []FunctionMetadata{
		...
		{"select_ast", 1, 3, `Every place a piece of syntax occurs in a file or tree, matched against the parse tree rather than the text (path, pattern, [options]). The pattern is code with holes: $NAME for one node, $$$NAME for a whole list of them, and $$$_ for "and anything else here" - a pattern says how many children a construct has, so f($A, $B) is a call with two arguments. Pass a list of patterns to search for all of them in one walk; each match names in Pattern which one found it. Files in a language no pattern is code in are skipped, so pass Language, or Include to one extension, to have the patterns checked rather than quietly skipped everywhere.`, "File Operations", []string{`select_ast("src"; "func $NAME($$$ARGS) error { $$$BODY }") | .Captures.NAME`, `[select_ast("src"; "if $C { $$$B }"; {Include: "*.go"})] | length`, `[select_ast("src"; ["md5.New()", "sha1.New()"])] | group_by(.Pattern) | map({(.[0].Pattern): length}) | add`, `select_ast("src"; "tls.Config{$$$_, InsecureSkipVerify: true, $$$_}")`}},
		{"ast_pattern", 1, 2, "What a code pattern compiles to, and whether it is code in that language at all - a pattern that is not still compiles, and then matches nothing in silence; Problem says why (pattern, language). This is what to reach for when scan_ast or select_ast comes back empty, because a typo and an honest absence look alike from the outside", "Code Rules", []string{`ast_pattern("func $N($$$A) error { $$$B }"; "go") | .Valid`, `"$X.$M($$$A)" | ast_pattern("go") | .MetaVariables`, `ast_pattern("md5($X)"; "php") | .Problem`}},
		...
		// Rules over that search
		{"invoke_pwrgrep", 1, 2, `Run structural rules over a tree and report what they find (path, rules). A rule is what select_ast is not: it carries the patterns, the guards that combine them, the files it is about and the sentence to say, so running one is naming it. Rules may be named by language, by finding id, by a glob over ids, or by a path into the catalogue that get_pwrgrep_rule lists; naming nothing that exists is an error, because "no rule called that" and "your code is clean" must not look alike. Name the language to run a language's rules: ids are not prefixed with it, so "python-*" is a glob matching the few ids that happen to start that way rather than the Python corpus. A rule is an ordinary pwrq query in a file, so writing one is copying one into $PWRQ_RULES or ~/.config/pwrq/rules.`, "Code Rules", []string{`invoke_pwrgrep("src"; "go-weak-hash")`, `"src" | invoke_pwrgrep(["go", "python"])`, `[invoke_pwrgrep("src"; "go/lang/security")] | group_by(.RuleId) | map({(.[0].RuleId): length}) | add`}},
		{"write_pwrgrep_rule", 1, 2, `Save a rule of your own where the catalogue will find it, and report where it went (name, source). The name is a place in the catalogue - "mine/no-timeout" - not a path on disk; the directory is $PWRQ_RULES or the one under the config directory, and it is created if it is not there. The header is read and the query is compiled before anything is written, because a file that is not a rule does not fail to fire, it makes every later invoke_pwrgrep fail. The rule is live in the same process the moment it lands, so the next invoke_pwrgrep runs it: write, run, edit, run again, without restarting anything.`, "Code Rules", []string{`write_pwrgrep_rule("mine/no-timeout"; $source) | .File`, `"mine/no-timeout" | write_pwrgrep_rule($source) | .Ids`, `write_pwrgrep_rule("mine/no-timeout"; $source) | invoke_pwrgrep("src"; .Path)`}},
		{"get_pwrgrep_rule", 0, 1, "One object per structural rule pwrq can see, with the query it is written as - which is also how you write your own: copy one into $PWRQ_RULES or ~/.config/pwrq/rules and it is found beside the rules that shipped (optional language, id, glob or catalogue path)", "Code Rules", []string{`get_pwrgrep_rule("go-weak-hash")`, `[get_pwrgrep_rule] | length`, `[get_pwrgrep_rule("python")] | map(.Id)`}},

		// The operators a rule is written with. Each takes a list of matches
		// and returns one, so a rule is a pipeline from one scan_ast to one
		// report.
		{"scan_ast", 2, 3, `The search a rule starts from: every match of every pattern, under one path, in the files a glob names (path, globs, patterns). One call rather than one per pattern is the point - the tree is walked once and each file parsed once however many patterns there are, so a rule with seven alternatives costs what a rule with one costs. Naming the files is how a rule says which language it is about: a pattern is code in more languages than the one it was written for, so a Go rule over a tree with a vendored jquery.min.js in it would otherwise report findings there.`, "Code Rules", []string{`"src" | scan_ast("*.go"; ["md5.New()", "sha1.New()"])`, `scan_ast("src"; ["*.js", "*.ts"]; ["eval($X)"]) | length`}},
		{"of", 1, 2, "Keep the matches one pattern found, or that any pattern in a list found (matches, patterns)", "Code Rules", []string{`[{Pattern: "md5.New()", Path: "a.go"}] | of("md5.New()")`, `"src" | scan_ast("*.go"; ["md5.New()", "sha1.New()"]) | of(["md5.New()"])`}},
		{"within", 1, 2, "Keep the matches some match in the other list encloses - a real span inside a real span (matches, outer)", "Code Rules", []string{`[{Path: "a.go", Offset: 5, EndOffset: 9}] | within([{Path: "a.go", Offset: 0, EndOffset: 20}])`}},
		{"outside", 1, 2, "Keep the matches no match in the other list encloses; an empty list keeps everything, because there was nothing to be inside (matches, outer)", "Code Rules", []string{`[{Path: "a.go", Offset: 5, EndOffset: 9}] | outside([{Path: "a.go", Offset: 30, EndOffset: 40}])`}},
		{"in_files_with", 1, 2, "Keep the matches in files the other list also matched - the reading of \"inside\" that means \"this file imports that\", which written as within would match nothing (matches, other)", "Code Rules", []string{`[{Path: "a.go", Offset: 5, EndOffset: 9}] | in_files_with([{Path: "a.go", Offset: 0, EndOffset: 2}])`}},
		{"in_files_without", 1, 2, "Keep the matches in files the other list did not match (matches, other)", "Code Rules", []string{`[{Path: "a.go", Offset: 5, EndOffset: 9}] | in_files_without([{Path: "b.go", Offset: 0, EndOffset: 2}])`}},
		{"at_same_place", 1, 2, "Keep the matches that start and end exactly where a match in the other list does - two patterns that must both describe the same code (matches, other)", "Code Rules", []string{`[{Path: "a.go", Offset: 5, EndOffset: 9}] | at_same_place([{Path: "a.go", Offset: 5, EndOffset: 9}])`}},
		{"not_at", 1, 2, "Keep the matches no match in the other list describes, which is how a rule excludes a special case (matches, other)", "Code Rules", []string{`[{Path: "a.go", Offset: 5, EndOffset: 9}] | not_at([{Path: "a.go", Offset: 0, EndOffset: 2}])`}},
		{"where_capture", 2, 3, "Keep the matches whose named hole caught text matching a regex; a hole the pattern did not have caught nothing, so a misspelled name filters everything out rather than nothing (matches, hole, regex)", "Code Rules", []string{`[{Captures: {P: "/tmp/x"}}] | where_capture("P"; "^/tmp/")`}},
		{"where_capture_not", 2, 3, "Keep the matches whose named hole caught text the regex does not match (matches, hole, regex)", "Code Rules", []string{`[{Captures: {ALGO: "MD5"}}] | where_capture_not("ALGO"; "SHA-?256")`}},
		{"where_same", 2, 3, "Keep the matches whose two named holes caught the same text; a pattern can write one hole twice, so this is for two holes written apart that may have landed on the same thing anyway (matches, hole, hole)", "Code Rules", []string{`[{Captures: {A: "x", B: "x"}}] | where_same("A"; "B")`}},
		{"where_different", 2, 3, "Keep the matches whose two named holes caught different text (matches, hole, hole)", "Code Rules", []string{`[{Captures: {A: "x", B: "y"}}] | where_different("A"; "B")`}},
		{"scan_regex", 2, 3, `The search a rule starts from when what it looks for is text rather than syntax (path, globs, regexes). Not every rule is about a parse tree - a template with "{% autoescape off %}" in it is a real finding and no grammar calls that a construct - and what comes back is the same kind of value scan_ast produces, so every operator downstream works on it unchanged and a rule can mix the two. A named group becomes a hole of that name, so (?P<KEY>...) fills in $KEY in the message. RE2, so no backreferences and no lookaround.`, "Code Rules", []string{`"src" | scan_regex("*.html"; "\\{%\\s*autoescape\\s+off\\s*%\\}")`, `scan_regex("src"; ["*.yml", "*.yaml"]; "password:\\s*\\S+") | length`}},
		{"where_text", 1, 2, "Keep the matches whose own text matches a regex - a regex used as a guard beside a pattern rather than instead of one (matches, regex)", "Code Rules", []string{`[{Text: "getenv(\"PASSWORD\")"}] | where_text("PASSWORD|SECRET")`}},
		{"where_text_not", 1, 2, "Keep the matches whose own text the regex does not match (matches, regex)", "Code Rules", []string{`[{Text: "x = 1"}] | where_text_not("^//")`}},
		{"where_capture_ast", 2, 3, "Keep the matches whose named hole caught something that is itself a match for another pattern - a constraint on a hole written as syntax rather than as a regex (matches, hole, pattern)", "Code Rules", []string{`[{Language: "python", Captures: {ARG: "os.getenv(\"X\")"}}] | where_capture_ast("ARG"; "os.getenv($$$_)")`}},
		{"where_capture_entropy", 1, 2, "Keep the matches whose named hole caught a string random enough to be a secret rather than a placeholder (matches, hole)", "Code Rules", []string{`[{Captures: {V: "\"AKIAIOSFODNN7EXAMPLE\""}}] | where_capture_entropy("V")`}},
		{"where_capture_redos", 1, 2, "Keep the matches whose named hole caught a regex an input can make a backtracking engine run for ever on (matches, hole)", "Code Rules", []string{`[{Captures: {R: "(a+)+$"}}] | where_capture_redos("R")`}},
		{"where_capture_compare", 1, 2, "Keep the matches whose named hole caught a number standing in some relation to another (matches, comparison). The hole's text is read in whatever base it was written in, so 0o777, 0x1FF and 511 are one value, and quotes around it are ignored", "Code Rules", []string{`[{Captures: {MODE: "0o777"}}] | where_capture_compare("$MODE >= 0o666")`}},
		{"focus", 1, 2, "Report the hole rather than the construct it was found in, moving where the finding points without changing which code it found (matches, hole)", "Code Rules", []string{`[{LineNumber: 1, Captures: {T: "True"}, CaptureSpans: {T: {LineNumber: 2, Column: 27}}}] | focus("T")`}},
		{"reaching", 2, 3, `Keep the sinks that a source flows into (sinks, sources, sanitizers). This is what a rule needs when neither end is a finding on its own: reading a query parameter is what a web application does and running a command is what a program does, and the finding is that the one reached the other. The following is intraprocedural and syntactic - it knows assignment, which is what carries a value from one line to the next in every language here - so it reports less than a whole-program analysis would rather than guessing.`, "Code Rules", []string{`"src" | scan_ast("*.py"; ["request.args.get($$$_)", "subprocess.run($$$_)"]) as $all | ($all | of(["subprocess.run($$$_)"]) | reaching($all | of(["request.args.get($$$_)"]); []))`}},
		{"finding", 2, 3, "Render matches as findings (matches, rule id, message). The message is a template: $NAME in it is replaced by what that hole in the pattern caught, and a name no hole has is left as written", "Code Rules", []string{`[{Path: "a.go", LineNumber: 3, Text: "md5.New()", Captures: {}}] | finding("go-weak-hash"; "this hash is not collision resistant")`, `[{Path: "a.java", LineNumber: 7, Text: "x", Captures: {ALGO: "MD5"}}] | finding("java-weak-crypto"; "$ALGO has known breaks")`}},
		{"report", 0, 1, "Put findings in the order a person reads them, and drop the duplicates two alternatives produce when both describe the same code (findings)", "Code Rules", []string{`[{RuleId: "a", Path: "z.go", LineNumber: 2, Column: 1, Match: "x"}, {RuleId: "a", Path: "a.go", LineNumber: 1, Column: 1, Match: "y"}] | report`}},
		...
		{"invoke_systemone", 1, 3, "Ask a System One model typed questions (noul, choice, score) about a state and return the answers, with probabilities, by question id", "LLM", []string{`"my card was charged twice" | invoke_systemone({refund: {type: "noul", instructions: "Does the customer want a refund?"}}) | .refund.noul`, `invoke_systemone({ticket: .}; {team: {type: "choice", criteria: {billing: "Payments", tech: null}}}; {Model: "systemone/jev-latest"}) | .team.choice`}},
		{"invoke_systemone_request", 1, 3, "The same call as invoke_systemone, reported as an object with the answers, served model, tokens and cost", "LLM", []string{`invoke_systemone_request({q: {type: "score", criteria: ["low", "high"]}}) | {Answers, TotalTokens}`}},
