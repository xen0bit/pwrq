...
func allFunctionMetadata() []FunctionMetadata {
	return []FunctionMetadata{
		...
		{"get_pwrgrep_rule", 0, 1, "One object per structural rule pwrq can see, with the query it is written as - which is also how you write your own: copy one into $PWRQ_RULES or ~/.config/pwrq/rules and it is found beside the rules that shipped (optional language, id, glob or catalogue path)", "Code Rules", []string{`get_pwrgrep_rule("go-functions")`, `[get_pwrgrep_rule] | length`, `[get_pwrgrep_rule("python")] | map(.Id)`}},
		...
		{"finding", 2, 3, "Render matches as findings (matches, rule id, message). The message is a template: $NAME in it is replaced by what that hole in the pattern caught, and a name no hole has is left as written", "Code Rules", []string{`[{Path: "a.go", LineNumber: 3, Text: "md5.New()", Captures: {}}] | finding("go-weak-hash"; "this hash is not collision resistant")`, `[{Path: "a.java", LineNumber: 7, Text: "run", Captures: {NAME: "run"}}] | finding("java-functions"; "method $NAME")`}},
