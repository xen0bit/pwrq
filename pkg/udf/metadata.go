...
func GetFunctionMetadata() []FunctionMetadata {
	return []FunctionMetadata{
		...
		{"ast_pattern", 1, 2, "What a code pattern compiles to, and whether it is code in that language at all - a pattern that is not still compiles, and then matches nothing in silence; Problem says why (pattern, language)", "File Operations", []string{`ast_pattern("func $N($$$A) error { $$$B }"; "go") | .Valid`, `"$X.$M($$$A)" | ast_pattern("go") | .MetaVariables`, `ast_pattern("md5($X)"; "php") | .Problem`}},
		...
		{"invoke_pwrgrep", 1, 2, `Run structural rules over a tree and report what they find (path, rules). A rule is what select_ast is not: it carries the patterns, the guards that combine them, the files it is about and the sentence to say, so running one is naming it. Rules may be named by finding id, by a glob over ids, or by a path into the catalogue that get_pwrgrep_rule lists; naming nothing that exists is an error, because "no rule called that" and "your code is clean" must not look alike. A rule is an ordinary pwrq query in a file, so writing one is copying one into $PWRQ_RULES or ~/.config/pwrq/rules.`, "Code Rules", []string{`invoke_pwrgrep("src"; "go-weak-hash")`, `"src" | invoke_pwrgrep(["go-*", "python-*"])`, `[invoke_pwrgrep("src"; "go/lang/security")] | group_by(.RuleId) | map({(.[0].RuleId): length}) | add`}},
		{"get_pwrgrep_rule", 0, 1, "One object per structural rule pwrq can see, with the query it is written as - which is also how you write your own: copy one into $PWRQ_RULES or ~/.config/pwrq/rules and it is found beside the rules that shipped (optional id, glob or catalogue path)", "Code Rules", []string{`get_pwrgrep_rule("go-weak-hash")`, `[get_pwrgrep_rule] | length`, `[get_pwrgrep_rule("python-*")] | map(.Id)`}},
