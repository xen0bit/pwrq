...
var (
	...
	ArchiveEntry = shape.Fixed("System.IO.Compression.ArchiveEntry",
	...
	//
	// It called itself a System.IO.FileInfo and is not one: there is no Mode,
	// Extension or IsHidden here, so a caller who looked the name up would be
	// told about properties that are not present.
	ArchiveWritten = shape.Fixed("Pwrq.Archive.WriteResult",
		...
		shape.OptProp("PSPath", shape.String, "the destination as given, as the bindable value"),
	)
)
