...
func RegisterSHA224() gojq.CompilerOption {
	return gojq.WithFunction("sha224", 0, 2, func(v any, args []any) any {
		...
		if err != nil {
			return fmt.Errorf("sha224: %v", err)
		}
		...
		if isFile {
			...
			if !ok {
				return fmt.Errorf("sha224: file argument requires string path, got %T", inputVal)
			}
			...
			case io.Reader:
				...
				if err != nil {
					return fmt.Errorf("sha224: failed to read input: %v", err)
				}
			...
			default:
				if str, ok := val.(fmt.Stringer); ok {
					...
					return fmt.Errorf("sha224: argument must be a string or bytes, got %T", val)
				}
			}
		}
