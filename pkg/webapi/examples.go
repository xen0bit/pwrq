...
| {path: ($p | join(".")), entropy: ($v | entropy)}`,
 ...
 {"name":"alan","pay":95},{"name":"ken","pay":85}]`,
		...
		},
		{
			...
			Description: "Named unit converters read a number and return the converted one.",
			Category:    "Domain",
			Query:       `{f: (100 | c_to_f), c: (212 | f_to_c), k: (0 | c_to_k)}`,
			Input:       `null`,
		},
		{
		...
		{
			Title:       "Pull matches out of a line",
			Description: "regex_find_all extracts every match, and regex_extract_first takes a capture group.",
			Category:    "String",
			Query:       `{ips: regex_find_all("[0-9]{1,3}(\\.[0-9]{1,3}){3}"), user: regex_extract_first("user=(\\w+)"; 1)}`,
			Input:       `"login from 10.0.0.1 as user=ada, retry from 10.0.0.2"`,
		},
		{
			Title:       "Split on a pattern",
			Description: "regex_split divides a string by a regex, where split takes a literal.",
			Category:    "String",
			Query:       `"a,b;c d" | regex_split("[,; ]")`,
			Input:       `null`,
		},
		{
			Title:       "Palindromes",
			Description: "is_palindrome ignores case and punctuation, so a famous phrase passes.",
			Category:    "String",
			Query:       `["racecar", "hello", "A man a plan a canal panama"] | map({word: ., palindrome: is_palindrome})`,
			Input:       `null`,
		},
		{
			Title:       "Clean up text",
			Description: "remove_accents and dedent tidy text: café becomes cafe and indentation is stripped.",
			Category:    "String",
			Query:       `{plain: ("café au lait" | remove_accents), dedented: ("    a\n      b" | dedent)}`,
			Input:       `null`,
		},
		{
			Title:       "Trim to word count",
			Description: "truncate_words cuts a long message to a headline with an ellipsis.",
			Category:    "String",
			Query:       `"the quick brown fox jumps over the lazy dog" | truncate_words(4)`,
			Input:       `null`,
		},
		{
		...
		{
			Title:       "Capture groups from every match",
			Description: "regex_groups returns each match's capture groups as an array of arrays.",
			Category:    "String",
			Query:       `"name=ada age=36 name=grace age=99" | regex_groups("(\\w+)=(\\d+)")`,
			Input:       `null`,
		},
		{
		...
		{
			Title:       "Rename and invert object keys",
			Description: "rename_keys and invert_object reshape an object without touching its values.",
			Category:    "Collections",
			Query:       `{"first_name":"ada","role":"admin"} | rename_keys({"first_name": "name"}) as $r | {renamed: $r, inverted: invert_object}`,
			Input:       `null`,
		},
		{
			Title:       "Pull a column of objects",
			Description: "pluck reads a property from every row, following dot paths.",
			Category:    "Collections",
			Query:       `pluck("metrics.cpu")`,
			Input: `[{"host":"web-01","metrics":{"cpu":12}},
 {"host":"db-01","metrics":{"cpu":63}},
 {"host":"web-02","metrics":{"cpu":4}}]`,
		},
		{
			Title:       "Autocorrelation of a series",
			Description: "autocorrelation measures how a series predicts itself, one lag at a time.",
			Category:    "Statistics",
			Query:       `[1,2,3,4,5,6,7,8] | {lag1: autocorrelation, lag2: autocorrelation(2)}`,
			Input:       `null`,
		},
		{
			Title:       "Spread and spread-of-variation",
			Description: "spread, iqr and mad each measure how far a set of values ranges.",
			Category:    "Statistics",
			Query:       `[2,4,4,4,5,5,7,9] | {spread: spread, iqr: iqr, mad: mad}`,
			Input:       `null`,
		},
		{
			Title:       "Week and day arithmetic",
			Description: "start_of_week, day_of_year and week_of_year place a timestamp on the calendar.",
			Category:    "Duration",
			Query:       `"2026-08-13T12:00:00Z" | {startOfWeek: (start_of_week | todateiso8601), dayOfYear: day_of_year, week: week_of_year}`,
			Input:       `null`,
		},
		{
		...
		},
		{
			...
			Description: "pairs walks an array in adjacent twos, handy for change detection.",
			Category:    "Collections",
			Query:       `["start", "running", "stopped", "start"] | pairs`,
			Input:       `null`,
		},
		{
			Title:       "Subset checks",
			Description: "is_subset tells you whether one list is contained in another.",
			Category:    "Collections",
			Query:       `{required: (["http", "ssh"] | is_subset(["http", "ssh", "smtp"])), missing: (["ftp"] | is_subset(["http", "ssh", "smtp"]))}`,
			Input:       `null`,
		},
		{
			Title:       "Write into a document by path",
			Description: "set_path, get_path and del_path address a document with dot-and-bracket paths.",
			Category:    "JSON",
			Query:       `{config: {hosts: ["a", "b"]}} | set_path("config.hosts[1]"; "c") | {hosts: get_path("config.hosts"), removed: (del_path("config.hosts[0]") | .config.hosts)}`,
			Input:       `null`,
		},
		{
			Title:       "Numbers as words",
			Description: "to_words spells a total out, the way an invoice does.",
			Category:    "Numbers",
			Query:       `[1, 42, 1234, 2026] | map(to_words)`,
			Input:       `null`,
		},
		{
			Title:       "Roman numerals",
			Description: "roman_numeral renders years and chapter numbers the old way.",
			Category:    "Numbers",
			Query:       `[4, 9, 40, 399, 2026] | map(roman_numeral)`,
			Input:       `null`,
		},
		{
		...
		},
		{
			...
			Description: "collatz_steps counts how long the hailstone sequence takes to reach 1.",
			Category:    "Numbers",
			Query:       `[1, 6, 27, 97] | map({n: ., steps: collatz_steps})`,
			Input:       `null`,
		},
		{
