...
func TestCliRun(t *testing.T) {
	...
	if err := os.Setenv("PWRQ_COLORS", ""); err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			for _, env := range tc.Env {
				...
				if k == "PWRQ_COLORS" || k == "GOJQ_COLORS" {
			...
			if tc.Error == "" {
				...
				// For version and help, we need to check if the output contains the expected text
				// since the exact format might vary slightly
				if strings.Contains(tc.Name, "version") || strings.Contains(tc.Name, "help") {
					if !strings.Contains(outStream.String(), "pwrq") {
						t.Error("standard output should contain 'pwrq'")
					}
				} else {
